# Colloids-project--Contact-angle-
Contact Angle and Contact Radius Dynamics of a Drying Droplet: Evaporation Rates, Surface Tension Analysis, and Force Balance

## 📝 Project Overview
We report a comprehensive experimental and theoretical study of evaporation dynamics of sessile droplets
on hydrophilic (glass) and hydrophobic (oil-coated glass) substrates at 30 ◦C and 45 ◦C using smartphone side-view
imaging. We derive and apply analytical expressions for the volumetric evaporation rate |dV /dt|, the instantaneous
drying rate, and the full force balance at the three-phase contact line, including solid–vapour, solid–liquid, and
liquid–vapour surface tension components. For water droplets on glass, evaporation proceeds primarily in the
Constant Contact Radius (CCR) mode with a nearly constant drying rate, while oil-coated substrates exhibit
Constant Contact Angle (CCA) behaviour with a rapidly decaying rate. Temperature elevation from 30 ◦C to 45 ◦C
accelerates evaporation by ≈ 40% and shifts the CCR-to-CCA transition earlier. Ethanol droplets confirm the same
qualitative behaviour with ≈ 2.5× faster drying. The diffusion-limited evaporation framework, particularly the
geometric factor f(θ), is shown to be the dominant controller of evaporation rate.

## Drive Reports and Experimental Readings

* 📄 **Full Report and experimental videos:** [View Report Document](https://drive.google.com/drive/folders/1RXzXpX4Z2a-q5e1aCOsQnQEUD751E6X-?usp=drive_link)
* 📊 **Project Presentation:** [View Slides]]((https://canva.link/0q0s1ks9t1pfgok))


